...
class HtmlReporter(BaseReporter):
    def report(self, report: ImpactReport, output_path: str = "report.html"):
        ...
        for it in report.impacted_tests:
            ...
            path_html = "".join([f"<div class='path-arrow'>▼ calls</div><div class='path-node {'path-changed' if i==0 else ('path-test' if i==len(it.call_path)-1 else 'path-helper')}'>{( '<span class=\"badge badge-changed\">CHANGED</span>' if i==0 else ('<span class=\"badge badge-test\">@Test</span>' if i==len(it.call_path)-1 else '<span class=\"badge badge-middle\">helper</span>'))} {node}</div>" for i, node in enumerate(it.call_path)])
